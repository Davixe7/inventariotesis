    ...
    <form action="{{route('venta.store')}}" method="POST">
      ...
      <div class="form-group row border">
        ...
        <div class="col-md-8">
          ...
          <select class="form-control selectpicker" name="id_equipo" id="id_equipo" data-live-search="true" required>

            <option value="0" selected>Seleccione</option>

            @foreach($equipos as $prod)

            <option value="{{$prod->id}}_{{$prod->stock}}_{{$prod->precio_venta}}">{{$prod->equipo}}</option>
          ...
          <table id="detalles" class="table table-bordered table-striped table-sm">
            ...
            <tfoot>
              ...
              <tr>
                ...
                <th>
                  <p align="right"><span align="right" id="total_pagar_html">USD$ 0.00</span> <input type="hidden" name="total_pagar" id="total_pagar"></p>
                </th>
              </tr>
...
<script>
  ...
  subtotal = [];
  $("#guardar").hide();
  $("#id_equipo").change(mostrarValores);

  function mostrarValores() {

    datosProducto = document.getElementById('id_equipo').value.split('_');
  ...
  function agregar() {
    ...
    id_equipo = datosProducto[0];
    equipo = $("#id_equipo option:selected").text();
    ...
    if (id_equipo != "" && cantidad != "" && cantidad > 0 && descuento != "" && precio_venta != "") {
      ...
      if (parseInt(stock) >= parseInt(cantidad)) {
        ...
        var fila = '<tr class="selected" id="fila' + cont + '"><td><button type="button" class="btn btn-danger btn-sm" onclick="eliminar(' + cont + ');"><i class="fa fa-times fa-2x"></i></button></td> <td><input type="hidden" name="id_equipo[]" value="' + id_equipo + '">' + equipo + '</td> <td><input type="number" name="precio_venta[]" value="' + parseFloat(precio_venta).toFixed(2) + '"> </td> <td><input type="number" name="descuento[]" value="' + parseFloat(descuento).toFixed(2) + '"> </td> <td><input type="number" name="cantidad[]" value="' + cantidad + '"> </td> <td>$' + parseFloat(subtotal[cont]).toFixed(2) + '</td></tr>';
        cont++;
        limpiar();
        totales();
