    ...
    <form action="{{route('compra.store')}}" method="POST">
      ...
      <div class="form-group row border">
        ...
        <div class="col-md-8">
          ...
          <select class="form-control selectpicker" name="id_equipo" id="id_equipo" data-live-search="true">

            <option value="0" selected>Seleccione</option>

            @foreach($equipos as $prod)

            <option value="{{$prod->id}}">{{$prod->equipo}}</option>
...
<script>
  ...
  function agregar() {
    ...
    if (id_equipo != "" && cantidad != "" && cantidad > 0 && precio_compra != "") {

      subtotal[cont] = cantidad * precio_compra;
      total = total + subtotal[cont];

      var fila = '<tr class="selected" id="fila' + cont + '"><td><button type="button" class="btn btn-danger btn-sm" onclick="eliminar(' + cont + ');"><i class="fa fa-times fa-2x"></i></button></td> <td><input type="hidden" name="id_equipo[]" value="' + id_equipo + '">' + equipo + '</td> <td><input type="number" id="precio_compra[]" name="precio_compra[]"  value="' + precio_compra + '"> </td>  <td><input type="number" name="cantidad[]" value="' + cantidad + '"> </td> <td>$' + subtotal[cont] + ' </td></tr>';
      cont++;
      limpiar();
      totales();
      ...
      // alert("Rellene todos los campos del detalle de la compra, revise los datos del equipo");
